...
import { 
  ...
  Clipboard, ImageIcon, Link2, UploadCloud, Video
} from "lucide-react";
...
const EXAM_BULK_QUESTION_TEMPLATE = `Savol: Yakuniy imtihonda laboratoriya xavfsizligining asosiy maqsadi nima?
A) Ish jarayonini sekinlashtirish
B) Talaba va namunalarni xavfsiz himoya qilish
C) Reagentlarni ko'paytirish
D) Hisobotni qisqartirish
Javob: B
Izoh: Laboratoriya xavfsizligi xodim, talaba, namuna va muhitni himoya qilishga qaratiladi.

Savol: Klinik laboratoriyada natijani tasdiqlashdan oldin nima tekshiriladi?
A) Faqat talabaning ismi
B) Asbob sozligi, namuna sifati va nazorat natijalari
C) Faqat sana
D) Faqat xona harorati
Javob: B
Izoh: Yakuniy natija sifat nazorati va namuna holati tekshirilgandan keyin tasdiqlanadi.`;
...
export function ExamsPage() {
  ...
  const handleSaveQuestion = async () => {
    ...
    if (questionType !== "text" && !mediaUrl.trim()) {
      toast.error(questionType === "image" ? "Rasm URL kiriting yoki fayl yuklang" : "Video URL kiriting yoki fayl yuklang");
      return;
    }
  ...
  const handleBulkImport = async () => {
    if (!formModuleId) {
      toast.error("Avval modulni tanlang");
      return;
    }
    const parsed = parseBulkQuestions();
    if (!parsed.length) {
      toast.error("Copy-paste formatidan imtihon savollari topilmadi");
      return;
    }
    ...
    try {
      ...
      setBulkText("");
      toast.success(`${parsed.length} ta matnli imtihon savoli import qilindi`);
    } catch (err: any) {
  ...
  const handleQuestionMediaUpload = async (
    ...
    if (isImageQuestion && !file.type.startsWith("image/")) {
      toast.error("Rasmli imtihon savoli uchun rasm faylini tanlang");
      return;
    }
    if (isVideoQuestion && !file.type.startsWith("video/")) {
      toast.error("Video imtihon savoli uchun video faylini tanlang");
      return;
    }
  ...
  const copyBulkTemplate = async () => {
    await navigator.clipboard.writeText(EXAM_BULK_QUESTION_TEMPLATE);
    toast.success("Imtihon savollari shabloni clipboardga nusxalandi");
  };
    ...
    <div className="min-h-screen bg-slate-50/50 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
        ...
        <div className="md:col-span-8 lg:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white border border-slate-200 shadow-sm rounded-lg p-4 flex flex-col justify-center items-center text-center col-span-2 sm:col-span-2">
                  ...
                  <tr key={exam.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="relative size-12 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 shadow-sm border border-slate-100 group-hover:shadow-md transition-all">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                        ...
                        <Button 
                          ...
                          className="size-9 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 disabled:opacity-50"
                        >
        ...
        {wizardStep === 2 && (
          <div className="grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
            <div className="space-y-4">
              ...
              <Card className="border border-indigo-200 bg-indigo-50/10 rounded-lg p-4.5 space-y-3">
              ...
              {isAddingQuestion ? (
                <Card className="border border-blue-200 bg-blue-50/10 rounded-lg p-4.5 space-y-3">
                  <div className="flex justify-between items-center">
                      ...
                      <div className="mb-3 flex items-center justify-between gap-3">
                        <div>
                          <p className="text-[11px] font-black uppercase tracking-wide text-slate-800">
                            {questionType === "image" ? "Rasm manbasi" : "Video manbasi"}
                          </p>
                      ...
                      <div className="grid gap-3 md:grid-cols-[1fr_180px]">
                        <div className="grid gap-1">
                          <label className="text-[11px] font-bold text-slate-600">
                            {questionType === "image" ? "Rasm URL" : "Video/YouTube URL"} *
                          </label>
                          <Input
                            placeholder={questionType === "image" ? "https://.../image.png" : "https://youtube.com/watch?v=..."}
                            value={mediaUrl}
                        ...
                        <label className="mt-5 flex h-9 cursor-pointer items-center justify-center rounded-lg border border-dashed border-blue-300 bg-blue-50 px-3 text-[11px] font-black text-blue-700 transition hover:bg-blue-100">
                          ...
                          <input
                            type="file"
                            className="sr-only"
                            accept={questionType === "image" ? "image/*" : "video/*"}
                  ...
                  <div className="grid gap-1">
                    <label className="text-[11px] font-bold text-slate-600">Tushuntirish (ixtiyoriy)</label>
                    <Textarea
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <Button 
                      ...
                      disabled={mediaUploading || createQuestionMutation.isPending || updateQuestionMutation.isPending}
                    >
                      {(mediaUploading || createQuestionMutation.isPending || updateQuestionMutation.isPending) && (
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-slate-200 rounded-lg py-5 hover:bg-slate-50 hover:border-slate-300 transition cursor-pointer"
                >
              ...
              <div className="max-h-[220px] overflow-y-auto pr-1 space-y-2 edulab-scrollbar">
                {isQuestionsLoading ? (
                  ...
                  wizardQuestions.map((q: any, index: number) => (
                    <div key={q.id} className="p-3 border border-border bg-white rounded-lg flex gap-3 items-start justify-between">
            ...
            {/* Sidebar info */}
            <div className="space-y-4">
              <div className="rounded-xl border-2 border-dashed border-indigo-200 bg-indigo-50/40 p-5 transition-all hover:bg-indigo-50/80">
                <div className="mb-4 flex items-start justify-between gap-4">
                  <div>
                    <h4 className="flex items-center gap-2 text-sm font-black uppercase tracking-wide text-indigo-800">
                      <Clipboard className="size-5 text-indigo-600" />
                      Ommaviy import (Copy & Paste)
                    </h4>
                    <p className="mt-2 text-xs font-semibold leading-relaxed text-indigo-700/80">
                ...
                <div className="mb-3 flex flex-wrap gap-2">
                  <Button
                    type="button"
                    variant="secondary"
                    onClick={() => setBulkText(EXAM_BULK_QUESTION_TEMPLATE)}
                    className="h-9 rounded-lg border border-indigo-200 bg-white px-4 text-[11px] font-black text-indigo-700 shadow-sm hover:bg-indigo-50"
                  >
                    Shablonni qo'yish
                  </Button>
                  <Button
                ...
                <div className="relative">
                  <Textarea
                    ...
                    placeholder={EXAM_BULK_QUESTION_TEMPLATE}
                    className="h-64 resize-y border-indigo-200 bg-white/90 p-4 font-mono text-[12px] leading-relaxed shadow-inner placeholder:text-slate-300 focus-visible:ring-indigo-500"
                  />
                  {bulkText.length > 0 && (
                    <div className="absolute right-3 top-3 rounded bg-indigo-100 px-2 py-1 text-[9px] font-black text-indigo-700">
                ...
                <Button
                  ...
                  className="mt-4 h-11 w-full rounded-lg bg-indigo-600 text-sm font-black text-white hover:bg-indigo-700 shadow-md"
                >
              ...
              <div className="border border-border rounded-xl p-5 bg-slate-50/50 space-y-4">
                ...
                <ul className="text-xs font-semibold text-slate-500 space-y-2">
                  ...
                  <li className="flex gap-2">
                    <span className="text-indigo-500 shrink-0">✓</span>
                    Matnli savollarni shablon bilan ko'p miqdorda import qilish mumkin.
                  </li>
                  <li className="flex gap-2">
                    <span className="text-indigo-500 shrink-0">✓</span>
                    Rasmli va video imtihon savollari URL yoki PC'dan yuklangan fayl bilan ishlaydi.
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="space-y-5">
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <div className="border border-border rounded-lg p-5 bg-slate-50/50 space-y-4">
                ...
                <div className="mt-4 p-3.5 bg-emerald-50 border border-emerald-100 rounded-lg flex gap-2 items-start animate-pulse">
